...
{
        FILE *file;
        int score = set_score(width * height, percent_mines, time_difference, open_field, spielende);
...
{
        int result;
        int multiply = (strcmp("Win", spielende) == 0) ? 2 : 1;
        result = ((num_of_field * (int) percent_mines * open_field) / ((int) time + 1)) * multiply;
        return result;
}
